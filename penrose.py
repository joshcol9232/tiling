...
def get_indices(r, sigmas, symmetry=5, angle_offset=ANGLE_OFFSET):
    """
    Returns the indices for any point on the plane.
    [a j0, b j1, c j2, d j3, e j4] where a,b,c,d,e are integers.
    """
    # Define normal unit vectors for each of the sets. Normal to the lines so: angle + pi/2.
    es = [np.array([ np.cos( (j * 2 * np.pi/symmetry) + angle_offset + np.pi/2.0 ), np.sin( (j * 2 * np.pi/symmetry) + angle_offset + np.pi/2 ) ]) for j in range(symmetry)]
    print(es)
...
class Intersection:
    ...
    def find_surrounding_indices(self, sigmas, symmetry=5, angle_offset=ANGLE_OFFSET):
        ...
        point_indices = get_indices(self.r, symmetry, angle_offset)
        ...
        surrounding_indices = [np.zeros(5, dtype=int) for i in range(4)]
        surrounding_indices[]
...
print("ind:", get_indices( np.array([1, 0.0]), sigmas ))
